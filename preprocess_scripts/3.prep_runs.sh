...
$plink --$species --bfile ${basefolder}/${inputfile} --update-ids ${basefolder}/${outputfolder}/newpop.txt --thin $thin --make-bed --out ${basefolder}/${outputfolder}/${basefnm}_thin

## when FID == IID, this may give problems with detectRUNS (especially plot functions)
echo "2. Update population information"

cut -f1-2 -d' ' ${basefolder}/${outputfolder}/${basefnm}_thin.ped > ${basefolder}/${outputfolder}/temp1
